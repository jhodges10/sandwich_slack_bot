...
lunch_option = ["Fritzi","Grow","Edibol","Cafe Gratitude","Pie Hole","Wurstkurch","Cerveteca","Americano","Umami","Groundwork",]
...
def conversation():
    ...
    while True:
        ...
        time.sleep(1)
        print cur_message
        if check_message_type(cur_message) <= 1:
            print "this is a cut or scene"
        elif check_message_type(cur_message) == 2:
            lunch_message = "You should have " +str(lunch_option[random.randint(0,9)]) + " for lunch."
            send_to_slack(lunch_message)
        user1 = User(cur_message[0],cur_message[1], '', '')
#        User.print_user_info(user1)
